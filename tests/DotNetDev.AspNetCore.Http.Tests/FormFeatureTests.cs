...
namespace DotNetDev.AspNetCore.Http.Tests;
...
public class FormFeatureTests
{
	...
	public async Task ReadFormAsync_0ContentLength_ReturnsEmptyForm()
	{
		...
		var formCollection = await context.Request.ReadFormAsync(TestContext.Current.CancellationToken);
	...
	public async Task FormFeatureReadsOptionsFromDefaultHttpContext()
	{
		...
		var exception = await Assert.ThrowsAsync<InvalidDataException>(() => context.Request.ReadFormAsync(TestContext.Current.CancellationToken));
	...
	public async Task ReadFormAsync_SimpleData_ReturnsParsedFormCollection(bool bufferRequest)
	{
		...
		// Cached
		formFeature = context.Features.GetRequiredFeature<IFormFeature>();
	...
	public async Task ReadFormAsync_SimpleData_ReplacePipeReader_ReturnsParsedFormCollection(bool bufferRequest)
	{
		...
		var pipe = new Pipe();
		await pipe.Writer.WriteAsync(formContent, TestContext.Current.CancellationToken);
...
"Foo\r\n";
	...
	public async Task ReadForm_MultipartWithField_ReturnsParsedFormCollection(bool bufferRequest)
	{
		...
		Assert.NotNull(formFeature.Form);
		Assert.Same(formCollection, formFeature.Form);
		Assert.Same(formCollection, await context.Request.ReadFormAsync(TestContext.Current.CancellationToken));
